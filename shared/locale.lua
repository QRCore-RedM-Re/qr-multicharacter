-- ==================== QR-MULTICHARACTER LOCALE SYSTEM ====================
local Locales = {}
local CurrentLocale = {}
    ...
    for _, lang in ipairs(availableLanguages) do
        ...
        if success and translations then
            ...
            if Config.Debug.enabled then
                print(('[QR-Multicharacter] Loaded %d translations for language: %s'):format(
                    #table.keys(translations), lang
                ))
            end
        else
            print(('[QR-Multicharacter] Failed to load translations for language: %s'):format(lang))
        end
    end
end
...
local function SetLocale(language)
    ...
    if Locales[targetLanguage] then
        ...
        if Config.Debug.enabled then
            print(('[QR-Multicharacter] Locale set to: %s'):format(targetLanguage))
        end
    else
        ...
        if Locales[fallback] then
            ...
            print(('[QR-Multicharacter] Language %s not found, using fallback: %s'):format(targetLanguage, fallback))
        else
            print(('[QR-Multicharacter] ERROR: Neither target language %s nor fallback %s found!'):format(targetLanguage, fallback))
            CurrentLocale = {}
        end
    end
end
...
function Locale(key, ...)
    if not CurrentLocale or not CurrentLocale[key] then
        if Config.Debug.enabled then
            print(('[QR-Multicharacter] Missing translation for key: %s'):format(key))
        end
...
function ChangeLanguage(newLanguage)
    ...
    if Locales[newLanguage] then
        ...
        if Config.Debug.enabled then
            print(('[QR-Multicharacter] Language %s not available'):format(newLanguage))
        end
        return false
    end
end
...
CreateThread(function()
    ...
    if Config.Debug.enabled then
        print('[QR-Multicharacter] Locale system initialized')
